        ...
        <li class="nav-item dropdown">
            <span class="nav-link text-muted pr-0 avatar-icon" id="navbarDropdownMenuLink" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                ...
                <div class="avatar-img rounded-circle avatar-initials-min text-center position-relative">
                    ...
                    $defaultUserImage = "assets/image/profile.png";
                    $defaultAdminImage = "assets/image/admin-default.png";
                    $imagePath = $defaultUserImage; // Default image
                    ...
                    if (isset($_SESSION['user_id'])) {
                        ...
                        if (mysqli_num_rows($select) > 0) {
                            ...
                            $imagePath = !empty($fetch['image']) ? "assets/image/" . $fetch['image'] : $defaultUserImage;
                        }
                    }
                    ...
                    if (isset($_SESSION['admin_id'])) {
                        ...
                        $select = mysqli_query($conn, "SELECT * FROM local_government_unit WHERE id = '$admin_id'") or die('Query failed');
                        if (mysqli_num_rows($select) > 0) {
                            $fetch = mysqli_fetch_assoc($select);
                            $imagePath = !empty($fetch['image']) ? "assets/image/" . $fetch['image'] : $defaultAdminImage;
                        }
                    }

                    echo '<img src="' . $imagePath . '" style="height: 100px; width: 100px; border-radius: 50%; object-fit: cover;">';
                    ?>
                </div>
                </span>
            </span>
